...
# ╔═╡ c6fde610-f932-4938-9564-5403e7a70ac6
# import packages
begin
...
# ╔═╡ b918a274-4e8d-4d08-941a-3e6acc90544a
begin
	using MLDataUtils: shuffleobs, stratifiedobs, rescale!
	using StatsFuns # we introduce this package so we can later call the logistic function directly instead of defining it manually as before
end

# ╔═╡ 799602f0-8e56-11ee-0505-abcfd2a2f33f
md"""
# Bayesian Logistic Regression
"""

# ╔═╡ 481f611a-f6fe-472c-85b8-f69b0743d23f
md"""
## Problem Statement
...
# ╔═╡ c5d9b157-725c-4917-a4a8-9f31df7d2fe8
md"""
## Data Exploration
...
# ╔═╡ 55a9a950-c17d-4f08-b638-6287eef2a2b8
# read the data using the CSV package
begin
...
# ╔═╡ 33eb1723-1a74-4d89-b8ad-0eb823290820
md"""
...
# ╔═╡ 70865942-6d76-42e0-a377-5007dd3eab83
md"""
...
# ╔═╡ 781a6403-f42a-49d2-89bc-645916db3492
md"""
### Data Interpretation
...
# ╔═╡ 73f09210-2eb9-4c62-960d-45da6544df79
begin
...
# ╔═╡ 35df8228-5b26-4cbf-8ba0-7e0e0299408b
md"""
## Model Specification
...
\beta_j &\sim \mathcal{N}(\mu_{\beta_j},\sigma_{\beta_j}) \\
\end{aligned}$$
...
# ╔═╡ ec7fc901-371b-4b52-8e9d-ea17422f183c
md"""
### Model Specification Using `Turing.jl`
"""

# ╔═╡ 917b5037-4318-488d-a49c-25756aaa958d
...
# ╔═╡ 8ef4cc5e-1f32-40d5-bd53-e31a4cf7ba01
md"""
#### Crank up the Bayes!
...
# ╔═╡ 2c3030bd-6160-43c3-a2c6-8803e5e058d9
...
# ╔═╡ 52ccfae2-d57f-437b-a39a-4baf4373712e
md"""
_NOTE_: The above routine employs the `MCMCThreads()` method to sample multiple chains. However, to implement this, one needs to change the environment variables for the number of threads Julia can use. These two discussions might shed some light as to how to achieve this:
...
# ╔═╡ 5dec4854-1f23-4acf-9729-a50363a01cc2
md"""
##### Plot the chains:
"""

# ╔═╡ 98ec7b01-5aca-4a86-ae03-82ced18d433d
plot(chain)

# ╔═╡ fcb5fb7f-51a3-412b-9e85-8193e660c87c
md"""
##### Get the summary statistics:
"""

# ╔═╡ 06e594be-a9e8-4248-9e02-530f12230156
describe(chain)

# ╔═╡ 75d54cd5-f88f-4e16-b653-4a5fedc6ed51
md"""
...
Again, below wecan see a collection of possible outcomes given the parameter sample space. There is a darker region where most sigmoid functions turned out, and these tend to be the most probable sigmoid functions, or, in other words, these sigmoid functions are the most probable functions that could fit the data, considering the distributions of the parameters too!
"""

# ╔═╡ 75d6f17e-22f2-4dd1-9b5f-e292edfb140b
begin
	x_line = 15:0.01:max_age
	...
	for i in 1:samples
		...
		plot!(p_data, x_line, p,
			legend = false,
			linewidth = 2, color= :blue, alpha = 0.02
		)
	end
...
# ╔═╡ ea829bb3-e613-45fe-9950-4993d4947a87
md"""
### Making Predictions
...
# ╔═╡ 30e39419-76ad-4951-99a2-c8d5a464ce3a
begin
...
# ╔═╡ 4d836d5c-521e-46d3-af6f-37f266510e0b
md"""
...
# ╔═╡ 7aa5dd70-b006-479c-b573-f93a217e299a
begin
	...
	for i in 1:length(new_Age)
		...
		pred_plot = scatter!(p_data, (new_Age[i], pred_mean))
	end
...
# ╔═╡ b57401cd-0b1a-448d-bee8-d48969c1b393
md"""
...
# ╔═╡ a99d2a53-2c6d-400c-9cd6-2acb24bdb5b7
begin
	function split_data(df, target; at=0.70)
	    shuffled = shuffleobs(df)
	    return trainset, testset = stratifiedobs(row -> row[target], shuffled; p=at)
...
# ╔═╡ 4f86aa27-b36e-4f4a-bf25-d663c6a63a4c
md"""
### Inference

Now that our data is formatted, we can perform our Bayesian logistic regression with multiple predictors: using chest pain (cp), age (age), resting bloodpressure (trestbps) and cholesterol (chol) levels.
"""

# ╔═╡ da9ba549-0984-423a-9f67-542bf173c5ee
...
# ╔═╡ 4842fe20-48f1-4455-81bb-68494e4e5f7c
begin
	X = train;
	y = train_label;
...
# ╔═╡ 6dd6373b-b522-4e56-a2bd-efa6d4e4d038
md"""
Now build the model and create the sample chain:
"""

# ╔═╡ 89834e77-66e2-4038-be69-7919cf9b458e
begin
	model_multi = logreg_multi(X, y)
	chain_multi = sample(model_multi, NUTS(), MCMCThreads(), 2_000, 8) # select 2000 samples directly
end

# ╔═╡ fab3af16-1df3-4d2e-a473-c66247779247
md"""
#### Summary Statistics
"""

# ╔═╡ f6675b27-f26e-4b13-985d-2ba01f5c0c4e
summarystats(chain_multi)

# ╔═╡ e1676a84-01fd-43aa-a06d-37a8a4f949ce
md"""
#### Plot the chains
"""

# ╔═╡ b2154c79-53df-4bda-b6e3-7da4641804a1
plot(chain_multi)

# ╔═╡ f0a15353-3569-43d5-92b9-df18ff55d6ea
md"""
#### Thank you!
...
manifest_format = "2.0"
project_hash = "a4e717c69299398ba878d641d3c5f63fc5d34c99"
