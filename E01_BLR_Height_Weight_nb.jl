...
# ╔═╡ ce052365-6a3d-4bc4-b720-db9773200e0e
# import the relevant packages
using LinearAlgebra, Turing, CSV, DataFrames, Plots, StatsPlots, LaTeXStrings

# ╔═╡ 67afb1be-34b6-43b7-934f-00f2a3f9a321
md"# Bayesian Linear Regression"

# ╔═╡ 1df189a0-8c13-11ee-31e3-41b9bd4ba3f0
md"""
### Finding a linear relationship between height and weight by Bayesian methods.
"""

# ╔═╡ 10c3f91e-6578-4b99-b5dc-26ec9bc3cad8
md"""
#### Problem statement
"""

# ╔═╡ b43c09c0-9aa5-4a48-86a3-c632b7e69bac
md"""
...
# ╔═╡ 7921fbc3-1ca4-455e-872a-9f0fc40eab60
md"""
#### Import the necessary packages
"""

# ╔═╡ b8735eb2-f122-40d8-8d5a-2e6cc60181d5
md"""
#### Bayesian Workflow
...
# ╔═╡ 39f45313-2a0c-4efa-b7e6-c2884afcef95
md"""
##### Collecting the data
...
# ╔═╡ cd47e3a7-1f89-4152-a718-0154cf37a9c8
# collect data
...
# ╔═╡ 2b4d87d1-4825-4b98-941e-c67e94fea2bf
md"""
For easier access, the column headers are changed as follows:
"""

# ╔═╡ e9735122-9335-45a1-a0ce-f27d44aeeaf1
# change the column headers for easier access

colnames = ["index","height","weight"]; rename!(df, Symbol.(colnames))

# ╔═╡ e79140ba-77ff-4d86-bb8c-ba2c6c9e3c17
md"""
##### Visualizing the data
"""

# ╔═╡ 47147811-0b37-4378-af30-efce96c36ab0
# plots
...
# ╔═╡ b0953875-d09a-4482-b10e-bdde2b850961
md"""
...
Therefore, we need to assign priors to the variables $\alpha$, $\beta$ and $\sigma^2$. The latter is a measure of the uncertainty in _the model_.
...
The last distribution is a _truncated normal distribution_ bounded from 0 to $\infty$. This is according to Gelman's suggestions on setting priors for variances.

"""

# ╔═╡ 787cd82d-218f-4f39-915c-d8fb373d3aaa
...
# ╔═╡ 1df647c6-fcec-423d-a619-aa2b678efc3f
md"""
The next step is to perform Bayesian inference:
"""

# ╔═╡ 779a7f17-be82-40d1-a4ae-c08996a3d0b2
# bayesian inference
model = blr(df.height, df.weight); samples = 500; chain = sample(model, NUTS(), samples)

# ╔═╡ 78320d3c-50fa-4c18-9440-297e4e0b2078
md"""

#### Visualizing the inference diagnostics
...
# ╔═╡ ed21d075-1e19-413f-aed1-9e24d4a1d3c7
md"""
##### Visualize the inference diagnostics
"""

# ╔═╡ ed4f865e-88d4-4794-95a4-2d5a4f69036c
plot(chain)

# ╔═╡ fe603ae9-6984-4833-b381-a93d77750d52
md"""
##### Summarize the results

"""

# ╔═╡ c57a77df-19bb-49a4-a7bd-3d589886810a
summarystats(chain)

# ╔═╡ c0381744-6e8f-46f6-8891-5dec42dc767d
md"""
...
# ╔═╡ 381cc48f-637a-4418-9af2-4c3b428f0a37
...
for i in 1:samples
	...
	plot!(plot_data, x -> α + β*x,
		...
		# error
		ribbon = σ², fillalpha = 0.003
	)
end	

# ╔═╡ 361c386e-5303-47de-8d39-e95284b74008
# ╠═╡ disabled = true
#=╠═╡
for i in 1:samples
	α = chain[i,1,1]    #chain[row, column, chain_ID]
...
# ╔═╡ 5dc266cd-7cdb-46ce-9429-21cadeee5c01
plot_data

# ╔═╡ 3434a917-c99a-4dd2-a07c-fc178ef64a82
md"""
##### Using the Linear Regression Model to Make Predictions
"""

# ╔═╡ 5750d691-81f1-45b3-bce7-260f70714966
md"""
Select the heights for which you want to predict the weights:
"""

# ╔═╡ c08c5d07-53c9-4223-a015-433462db8bc0
# sample from the predictive posterior distribution
# pick 6 different heights to see what the corresponding weights would be according to the linear predictor:

pred_height = [62, 84, 75, 70, 71, 67]

# ╔═╡ c785e1fa-9aa0-4075-93ed-8d84e988538c
md"""
Now, run the prediction command from `Turing.jl`:
"""

# ╔═╡ cea80700-8505-4c13-9b7c-a90518ba2c46
# predict the weights corresponding to the heights
# this function will result in a probability distribution for each of the weights,
# because Turing will plug in the pred_height values to all of the chains

predictions = predict(blr(pred_height, missing), chain)

# ╔═╡ 53a2d995-1e8e-4390-92fa-5c347111bab0
md"""
Visualize the probability distributions of the predictions:
"""

# ╔═╡ 68103cc3-6812-4591-86ce-61eea1db8777
# visualize the probability distributions of the predictions

plot(predictions)

# ╔═╡ ce30d2a6-fd89-47b9-82fb-b2a062f0d001
md"""
Finally, to obtain a point-estimate, compute the average weight prediction for each height:
"""

# ╔═╡ 894725e9-e7f7-4c92-aa8f-86783a6b0628
...
manifest_format = "2.0"
project_hash = "48a0383f8b6bc2acb549b87fa72d1b4baf777049"
